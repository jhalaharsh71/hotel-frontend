...
const TodaysCheckout = () => {
  ...
  /* =======================
     FETCH BOOKINGS WITH TODAY'S CHECKOUT
  ======================= */
  useEffect(() => {
    fetchTodaysCheckouts();
  }, []);

  const fetchTodaysCheckouts = async () => {
    try {
      setLoading(true);
      setError(null);
      ...
      // FILTER: Show only bookings with check-out date = today
      ...
      console.error(err);
      setError("Failed to load today's check-outs. Please try again.");
      setBookings([]);
    } finally {
      setLoading(false);
    }
    ...
    return (
      <Container className="d-flex justify-content-center align-items-center" style={{ minHeight: "100vh" }}>
        <div className="text-center">
          ...
          <p className="mt-3 text-muted">Loading today's check-outs...</p>
        </div>
      </Container>
    );
  }
    ...
    return (
      <Container className="py-5">
        <Alert variant="danger" className="d-flex gap-3">
          <AlertCircle />
          <div>
            <h5>Error Loading Check-outs</h5>
            <p>{error}</p>
            <Button variant="danger" onClick={fetchTodaysCheckouts}>
              Try Again
            </Button>
          </div>
        </Alert>
            ...
            {bookings.length === 0 ? (
              <div className="empty-state">
                ...
                <p className="text-muted">
                  There are no bookings with check-out date today.
                </p>
              </div>
            ) : (
